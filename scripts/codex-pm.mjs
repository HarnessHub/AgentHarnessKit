...
const VALID_TASK_TYPES = ["implementation", "docs", "ops", "research", "umbrella"];
...
function prdNew(args, io) {
  ...
  initPm();
  const filePath = path.join(PM_ROOT, "prds", `${slug}.md`);
...
function epicNew(args, io) {
  ...
  if (options.prd) {
    metadata.prd = options.prd;
  }
  const filePath = path.join(PM_ROOT, "epics", `${slug}.md`);
  writeDocument(filePath, metadata, {
    Outcome: "",
    Scope: "- ",
    "Acceptance Criteria": "- ",
    "Child Issues": "- ",
...
function taskNew(args, io) {
  ...
  initPm();
  const filePath = path.join(PM_ROOT, "tasks", epic, `${slug}.md`);
  const metadata = {
    type: "task",
    epic,
    slug,
  ...
  if (options.issue) {
    metadata.issue = options.issue;
  }

  writeDocument(filePath, metadata, buildTaskSections({
    title: options.title,
    issue: options.issue,
...
function listTasks(args, io) {
  ...
  const documents = loadTasks({ status: options.status, epic: options.epic });
  if (options.json) {
    io.log(JSON.stringify(documents.map(docToDict), null, 2));
    return 0;
  }
  for (const document of documents) {
    const issue = document.metadata.issue ? ` issue=${document.metadata.issue}` : "";
    io.log(`${document.path} status=${document.metadata.status ?? "backlog"}${issue}`);
  }
  return 0;
}
...
function nextTask(args, io) {
  const { options } = parseArgs(args);
  const documents = loadTasks({ epic: options.epic }).filter((document) => (document.metadata.status ?? "backlog") === "backlog");
  if (documents.length === 0) {
    io.log(options.json ? "null" : "No backlog task found.");
    return 0;
  }
  const document = documents[0];
  io.log(options.json ? JSON.stringify(docToDict(document), null, 2) : document.path);
  return 0;
}
...
function prBody(args, io) {
  ...
  requireValue(filePath, "path is required");
  const document = readDocument(filePath);
  io.log(renderPrBody(document, {
    issue: options.issue ? Number(options.issue) : undefined,
    tests: asArray(options.tests),
  }));
  return 0;
}
...
export function persistDocument(document) {
  ...
  for (const [heading, body] of Object.entries(document.sections)) {
    lines.push(`## ${heading}`, "");
    if (body) {
      lines.push(...String(body).replace(/\s+$/u, "").split("\n"));
    }
    lines.push("");
  }
...
function loadTasks({ status, epic }) {
  const taskRoot = path.join(PM_ROOT, "tasks");
  if (!fs.existsSync(taskRoot)) {
    return [];
  }

  const results = [];
  for (const epicDir of fs.readdirSync(taskRoot, { withFileTypes: true })) {
    if (!epicDir.isDirectory()) {
      continue;
    }
    if (epic && epicDir.name !== epic) {
      continue;
    }
    const epicPath = path.join(taskRoot, epicDir.name);
    for (const entry of fs.readdirSync(epicPath, { withFileTypes: true })) {
      if (!entry.isFile() || !entry.name.endsWith(".md")) {
        continue;
      }
      const document = readDocument(path.join(epicPath, entry.name));
      if (status && document.metadata.status !== status) {
        continue;
      }
...
function issueStatePath(taskDocument) {
  const issue = parseIssueNumber(taskDocument.metadata.issue ?? "");
  if (issue === null) {
    throw new Error(`task has no numeric issue reference: ${taskDocument.path}`);
  }
  const slug = taskDocument.metadata.slug ?? path.basename(taskDocument.path, ".md");
...
function loadIssueState(taskDocument) {
  const candidates = [];
  if (taskDocument.metadata.state_path) {
    candidates.push(taskDocument.metadata.state_path);
  }
  try {
    candidates.push(issueStatePath(taskDocument));
  } catch {
    // Ignore tasks without a numeric issue reference.
  }
...
function syncLinkedIssueStateStatus(taskDocument) {
  if ((taskDocument.metadata.type ?? "") !== "task") {
    return;
  }
  const stateDocument = loadIssueState(taskDocument);
  if (!stateDocument) {
...
function checkIssueState(branch) {
  ...
  const taskDocument = findTaskByIssue(issue);
  if (!taskDocument) {
    return {
      ok: false,
      message: `Issue state check failed: no task twin found for issue #${issue}.`,
    };
  }

  if ((taskDocument.metadata.status ?? "backlog") !== "in_progress") {
    return {
      ok: true,
      message: `Issue state check skipped: task for issue #${issue} is not in progress.`,
    };
  }
  ...
  if (!stateDocument) {
    return {
      ok: false,
      message: `Issue state check failed: in-progress issue has no state document. Run \`node scripts/codex-pm.mjs issue-state-init ${taskDocument.path}\`.`,
    };
  }

  if ((stateDocument.metadata.status ?? "") !== (taskDocument.metadata.status ?? "")) {
    return {
      ok: false,
      message: `Issue state check failed: task and issue-state status differ for issue #${issue}. Task=${taskDocument.metadata.status ?? ""} issue-state=${stateDocument.metadata.status ?? ""}.`,
    };
  }

  return {
    ok: true,
    message: `Issue state check passed: ${stateDocument.path}`,
  };
}
...
function checkDeliveryState(branch, { allowReadyToDeliver = false } = {}) {
  ...
  if (!taskDocument) {
    return {
      ok: false,
      message: `Delivery state check failed: no task twin found for issue #${issue}.`,
    };
  }

  if ((taskDocument.metadata.status ?? "backlog") !== "done") {
    return {
      ok: true,
      message: `Delivery state check skipped: task for issue #${issue} is not done.`,
    };
  }

  const stateDocument = loadIssueState(taskDocument);
  const statePath = stateDocument?.path ?? taskDocument.metadata.state_path ?? issueStatePath(taskDocument);
  const stage = stateDocument?.metadata.delivery_stage ?? defaultDeliveryStage(taskDocument);
  if (stage === "pr_opened") {
    return {
      ok: true,
      message: `Delivery state check passed: issue #${issue} is already at pr_opened.`,
    };
  }

  const openPrUrl = findOpenPrForBranch({
    baseRepo: inferBaseRepo(),
    headBranch: branch,
  });
  if (openPrUrl) {
    ...
    return {
      ok: true,
      message: `Delivery state check passed: issue #${issue} already has an open PR.`,
    };
  }
  ...
  if (allowReadyToDeliver) {
    return {
      ok: true,
      message: `Delivery state check allowed: issue #${issue} is ready_to_deliver without an open PR because the current flow is allowed to create one.`,
    };
  }

  return {
    ok: false,
    message: `Delivery state check failed: issue #${issue} is done but still has no open PR. Current delivery stage is ${stage} (${statePath}).`,
  };
}

function findOpenPrForBranch({ baseRepo, headOwner, headBranch }) {
  if (!baseRepo || !commandExists("gh")) {
    return null;
  }
...
export function verifyClosureSync(prBody, changedFiles) {
  ...
  for (const issue of [...new Set(closingIssues)]) {
    ...
    if (umbrella.length > 0) {
      errors.push(`PR closes #${issue} but matching task file is task_type=umbrella and must remain open: ${umbrella.map((document) => document.path).join(", ")}`);
      continue;
    }

    if (!matching.some((document) => document.metadata.status === "done")) {
      errors.push(`PR closes #${issue} but matching task file is not marked done: ${matching.map((document) => document.path).join(", ")}`);
      continue;
    }
...
function buildIssueStateSections(taskDocument) {
  ...
  return {
    Summary: context || "Record the current working state so later sessions do not need to rediscover it.",
...
function resolveChangedFiles(options) {
  if (options["changed-file"]) {
    return asArray(options["changed-file"]);
  }
